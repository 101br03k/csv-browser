        ...
        <Table>
          ...
          <TableBody>
            {currentPageData.length === 0 ? (
              <TableRow>
                <TableCell colSpan={visibleColumns.length} className="px-6 py-16 text-center text-sm text-gray-500">
              ...
              currentPageData.map((row, rowIndex) => (
                <TableRow key={rowIndex} className="hover:bg-gray-50">
                  {columns
                    .filter(column => visibleColumns.includes(column.id))
                    .map((column) => (
                      <TableCell key={column.id} className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {row[column.id]}
                      </TableCell>
                    ))}
                </TableRow>
        ...
        <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-gray-700">
              Showing
              <span className="font-medium mx-1">{data.length > 0 ? startIndex + 1 : 0}</span>
              to
                ...
                return (
                  <Button
                    ...
                    className={`relative inline-flex items-center px-4 py-2 border ${
                      currentPage === page 
                        ? "z-10 bg-primary text-white hover:bg-blue-600 border-primary" 
